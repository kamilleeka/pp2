...
class Background:  # Класс Background, который определяет фон игры и рисует сетку на экране.
...
class Food:  # Класс пищи, который представляет собой пищу, которую змея должна есть, чтобы расти дольше.
    def __init__(self):
        self.color = RED
        self.spawn()
...
class Snake: # Класс Body, который представляет каждый сегмент тела змеи.
...
class Collision:
    ...
    def snake_and_walls(self, snake, walls):  # если змейка ударяется об стену
