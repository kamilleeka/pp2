...
def r_triangle(color, start_pos,
               end_pos):  # на тетради рисуем график прямоугольного треугольника, и оттуда выводим формулу для
...
def romb(color, start_pos,
         end_pos):  # для ромба формулы нету к сожалению, методом подбора +- правильную величину вычисляем
    x1 = start_pos[0]
    x2 = end_pos[0]
...
def e_triangle(color, start_pos,
               end_pos):  # так же произошли трудности с правильным треугольником, +- вывел формулу и подставил  в pygame.draw.line
    x1 = start_pos[0]
    x2 = end_pos[0]
    y1 = start_pos[1]
    y2 = end_pos[1]
